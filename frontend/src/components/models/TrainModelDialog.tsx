...
import { Loader2, ArrowRight, ArrowLeft, ChevronsRight, ChevronsLeft } from "lucide-react"
...
export function TrainModelDialog({ open, onOpenChange, onSuccess, initialConfig }: TrainModelDialogProps) {
    ...
    useEffect(() => {
        ...
        selectedFsId, targetCol, selectedFeatures, experimentName,
        objective, metric, learningRate, numLeaves, boostingType,
        nClusters, initMethod,
        nEstimators, maxDepth, minChildSamples, featureFraction, baggingFraction, lambdaL1, lambdaL2,
        paramsJson, open
    ])
    ...
    const moveAllRight = () => {
        const allLeft = availableColumns.filter(c => c !== targetCol)
